...
const { currentDriver } = require('./storageDriver');
...
// Legacy layout: one array document per team. New layout: one document per draft
// plus a small index of ids, so a live-game autosave rewrites only the changed
// draft instead of the whole array.
const teamDraftsBlobName = (teamId) => `teams/${teamId}/drafts.json`; // legacy (array)
const teamDraftsIndexBlobName = (teamId) => `teams/${teamId}/drafts/index.json`;
const teamDraftBlobName = (teamId, draftId) => `teams/${teamId}/drafts/${draftId}.json`;
...
const teamDraftsFilePath = (teamId) => path.join(storeDir, 'teams', `${teamId}.drafts.json`); // legacy (array)
const teamDraftsIndexFilePath = (teamId) => path.join(storeDir, 'teams', teamId, 'drafts', 'index.json');
const teamDraftFilePath = (teamId, draftId) => path.join(storeDir, 'teams', teamId, 'drafts', `${draftId}.json`);
...
function assertPersistable() {
  if (currentDriver() === 'blob' && !process.env.BLOB_READ_WRITE_TOKEN) {
    throw new Error(
...
async function pgReadJson(blobName, fallback) {
  const raw = await pgKv.get(blobName);
  return raw == null ? fallback : decrypt(raw, fallback);
}

async function pgWriteJson(blobName, value) {
  await pgKv.set(blobName, encrypt(JSON.stringify(value)));
}

async function pgDeleteByName(blobName) {
  await pgKv.del(blobName);
}

// ---------- Backend-agnostic document I/O ----------
// Each logical document has a blob key (used by Postgres + Vercel Blob) and a
// local file path (used by the file backend). The active driver decides which.

async function readDoc(blobName, filePath, fallback) {
  const driver = currentDriver();
  if (driver === 'postgres') {
    try {
      return await pgReadJson(blobName, fallback);
    } catch (e) {
      console.error('Failed to read from Postgres:', blobName, e.message);
      return fallback;
    }
  }
  if (driver === 'blob') {
    try {
      return await blobReadJson(blobName, fallback);
    } catch (e) {
      console.error('Failed to read from Vercel Blob:', blobName, e.message);
      return fallback;
    }
  }
  return fileReadJson(filePath, fallback);
}

async function writeDoc(blobName, filePath, value) {
  const driver = currentDriver();
  if (driver === 'postgres') {
    await pgWriteJson(blobName, value);
    return;
  }
  if (driver === 'blob') {
    await blobWriteJson(blobName, value);
    return;
  }
  fileWriteJson(filePath, value);
}

async function deleteDoc(blobName, filePath) {
  const driver = currentDriver();
  if (driver === 'postgres') {
    await pgDeleteByName(blobName);
    return;
  }
  if (driver === 'blob') {
    await blobDeleteByName(blobName);
    return;
  }
...
async function readIndex() {
  const data = await readDoc(TEAMS_INDEX_BLOB, indexFilePath, []);
  return Array.isArray(data) ? data : [];
}

async function writeIndex(teams) {
  await writeDoc(TEAMS_INDEX_BLOB, indexFilePath, teams);
}
...
async function readTeamData(teamId) {
  ...
  const data = await readDoc(teamDataBlobName(teamId), teamDataFilePath(teamId), fallback);
  return normalizeData(data);
}

async function writeTeamData(teamId, data) {
  const clean = normalizeData(data);
  await writeDoc(teamDataBlobName(teamId), teamDataFilePath(teamId), clean);
  return clean;
}
...
function normalizeData(data) {
  ...
  const leagues = Array.isArray(out.leagues)
    ...
    : [];
  // Finished competitions: still counted in stats (games stay tagged) but hidden
  // from the new-game recorder. Only keep entries that exist in the registry.
  const leagueSet = new Set(leagues.map(l => l.toLowerCase()));
  const finishedLeagues = [];
  const seenFinished = new Set();
  if (Array.isArray(out.finishedLeagues)) {
    out.finishedLeagues
      .filter(l => typeof l === 'string' && l.trim())
      .map(l => l.trim())
      .forEach(l => {
        const key = l.toLowerCase();
        // Keep only registered competitions; de-dupe case-insensitively.
        if (leagueSet.has(key) && !seenFinished.has(key)) {
          seenFinished.add(key);
          finishedLeagues.push(l);
        }
      });
  }
  return {
...
// Read/write the small index of draft ids for a team. A null result means the
// index document does not exist yet (i.e. this team hasn't been migrated to the
// per-draft layout).
async function readDraftIndex(teamId) {
  const data = await readDoc(teamDraftsIndexBlobName(teamId), teamDraftsIndexFilePath(teamId), null);
  return Array.isArray(data) ? data.map(String) : (data === null ? null : []);
}

async function writeDraftIndex(teamId, ids) {
  const clean = Array.isArray(ids) ? Array.from(new Set(ids.map(String))) : [];
  await writeDoc(teamDraftsIndexBlobName(teamId), teamDraftsIndexFilePath(teamId), clean);
  return clean;
}

async function readOneDraft(teamId, draftId) {
  return readDoc(teamDraftBlobName(teamId, draftId), teamDraftFilePath(teamId, draftId), null);
}

async function writeOneDraft(teamId, draft) {
  await writeDoc(teamDraftBlobName(teamId, draft.id), teamDraftFilePath(teamId, draft.id), draft);
}

async function deleteOneDraft(teamId, draftId) {
  await deleteDoc(teamDraftBlobName(teamId, draftId), teamDraftFilePath(teamId, draftId));
}

// One-time migration from the legacy single-array document to per-draft docs +
// index. Memoized per process; the persisted index doubles as the durable flag.
const migratedDraftTeams = new Set();
async function ensureDraftsMigrated(teamId) {
  if (migratedDraftTeams.has(teamId)) return;
  const idx = await readDraftIndex(teamId);
  if (idx !== null) { migratedDraftTeams.add(teamId); return; }
  // No index yet: fan out any legacy array into per-draft docs. Only materialize
  // an index when there is legacy data to migrate; teams with no drafts create
  // the index lazily on their first save (avoids spurious empty index docs).
  const legacy = await readDoc(teamDraftsBlobName(teamId), teamDraftsFilePath(teamId), null);
  const arr = Array.isArray(legacy) ? legacy.filter((d) => d && d.id) : [];
  if (arr.length) {
    for (const d of arr) {
      await writeOneDraft(teamId, d);
    }
    await writeDraftIndex(teamId, arr.map((d) => String(d.id)));
  }
  if (legacy !== null) {
    try { await deleteDoc(teamDraftsBlobName(teamId), teamDraftsFilePath(teamId)); } catch (e) { /* best effort */ }
  }
  migratedDraftTeams.add(teamId);
}

/**
 * Serialize index mutations (add/remove draft ids) to limit lost updates from
 * bursty autosaves. The callback receives the current id array and returns
 * { ids, value }. Individual draft documents are written inside the callback.
 */
function mutateDraftIndex(teamId, fn) {
  const run = draftsWriteQueue.then(async () => {
    await ensureDraftsMigrated(teamId);
    const ids = (await readDraftIndex(teamId)) || [];
    const result = await fn(ids);
    await writeDraftIndex(teamId, result.ids);
    return result.value;
  });
...
async function deleteTeam(id) {
  ...
  if (removed) {
    ...
    try {
      await deleteDoc(teamDataBlobName(id), teamDataFilePath(id));
      // Legacy single-array drafts doc (pre per-draft layout).
      await deleteDoc(teamDraftsBlobName(id), teamDraftsFilePath(id));
      // Per-draft docs + index.
      const ids = (await readDraftIndex(id)) || [];
      for (const draftId of ids) await deleteOneDraft(id, draftId);
      await deleteDoc(teamDraftsIndexBlobName(id), teamDraftsIndexFilePath(id));
    } catch (e) {
      console.error('Failed to delete team data for', id, e.message);
    }
    migratedDraftTeams.delete(id);
  }
  return removed;
}
...
async function getDrafts(teamId) {
  ...
  const drafts = [];
  for (const id of ids) {
    const d = await readOneDraft(teamId, id);
    if (d) drafts.push(d);
  }
...
/** Get a single draft by id (or null). O(1): reads just that draft document. */
async function getDraft(teamId, draftId) {
  await ensureDraftsMigrated(teamId);
  return (await readOneDraft(teamId, draftId)) || null;
}
...
async function saveDraft(teamId, draft, { createdBy = null } = {}) {
  ...
  // Serialize only the index update; the (single) draft document is written
  // inside so bursty autosaves rewrite one small doc, not the whole array.
  return mutateDraftIndex(teamId, async (ids) => {
    const existing = await readOneDraft(teamId, id);
    let stored;
    if (!existing) {
      stored = { ...incoming, id, teamId, createdBy, createdAt: now, updatedAt: now };
    ...
    await writeOneDraft(teamId, stored);
    return { ids, value: stored };
  });
}
...
async function deleteDraft(teamId, draftId) {
  ...
  return mutateDraftIndex(teamId, async (ids) => {
    const existed = ids.includes(id) || (await readOneDraft(teamId, id)) != null;
    const next = ids.filter((x) => x !== id);
    if (existed) await deleteOneDraft(teamId, id);
    return { ids: next, value: existed };
  });
}
...
function getStorageInfo() {
  const driver = currentDriver();
  if (driver === 'postgres') {
    return { mode: 'postgres', configured: pgKv.configured(), indexKey: TEAMS_INDEX_BLOB };
  }
  if (driver === 'blob') {
    return {
      mode: 'vercel-blob',

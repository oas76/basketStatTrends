...
async function readDrafts(teamId) {
  const fallback = [];
  const data = await readDoc(teamDraftsBlobName(teamId), teamDraftsFilePath(teamId), fallback);
  return Array.isArray(data) ? data : fallback;
}

async function writeDrafts(teamId, drafts) {
  const clean = Array.isArray(drafts) ? drafts : [];
  await writeDoc(teamDraftsBlobName(teamId), teamDraftsFilePath(teamId), clean);
  return clean;
}

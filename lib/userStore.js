...
async function readFromStorage() {
  ...
  if (driver === 'postgres') {
    try {
      const raw = await pgKv.get(USERS_BLOB_NAME);
      return raw == null ? [] : decrypt(raw);
    } catch (e) {
      console.error('Failed to read user store from Postgres:', e.message);
      return [];
    }
  }
...
async function writeToStorage(users) {
  ...
  if (driver === 'postgres') {
    await pgKv.set(USERS_BLOB_NAME, payload);
    return;
  }
...
function getStorageInfo() {
  const driver = currentDriver();
  if (driver === 'postgres') {
    return { mode: 'postgres', configured: pgKv.configured(), key: USERS_BLOB_NAME };
  }
  if (driver === 'blob') {
    return {
      mode: 'vercel-blob',
...
const VALID_TEAM_ROLES = ['admin', 'member', 'recorder'];
// Platform-level roles. `recorder` can access the mobile game recorder for teams
// they are a member of, but has no admin/settings access.
const VALID_PLATFORM_ROLES = ['user', 'admin', 'recorder'];

/** Normalize a role input (string|array) to a deduped, valid roles array. */
function normalizeRoles(input) {
  let roles = [];
  if (Array.isArray(input)) roles = input.slice();
  else if (input) roles = [input];
  roles = roles
    .map(r => String(r || '').trim().toLowerCase())
    .filter(r => VALID_TEAM_ROLES.includes(r));
  roles = Array.from(new Set(roles));
  if (!roles.length) roles = ['member'];
  return roles;
}

/** Roles for a single membership entry, upgrading legacy { role } -> [role]. */
function membershipRoles(entry) {
  if (!entry) return [];
  if (Array.isArray(entry.roles)) return normalizeRoles(entry.roles);
  if (entry.role) return normalizeRoles(entry.role);
  return ['member'];
}

/** Upgrade a user's teams[] in place to the roles[] shape (legacy migration). */
function normalizeUserTeams(user) {
  if (!user) return;
  if (!Array.isArray(user.teams)) { user.teams = []; return; }
  user.teams = user.teams
    .filter(t => t && t.teamId)
    .map(t => ({ teamId: t.teamId, roles: membershipRoles(t) }));
}
...
async function createUser({ email, name = '', role = 'user', passwordHash = null, mustChange = false }) {
  ...
  if (!VALID_PLATFORM_ROLES.includes(role)) role = 'user';
...
async function updateUser(id, patch = {}) {
  return mutate((users) => {
    ...
    if (VALID_PLATFORM_ROLES.includes(patch.role)) user.role = patch.role;
...
/** Get a user's team memberships: [{ teamId, roles: [] }]. */
...
/** Get a user's roles[] for a specific team ([] if not a member). */
async function teamRolesOf(userId, teamId) {
  const user = await findById(userId);
  if (!user || !Array.isArray(user.teams)) return [];
  const entry = user.teams.find(t => t.teamId === teamId);
  return entry ? membershipRoles(entry) : [];
}

/** True if the user holds `role` on the team. */
async function hasTeamRole(userId, teamId, role) {
  const roles = await teamRolesOf(userId, teamId);
  return roles.includes(role);
}

/**
 * Add or update a user's membership of a team.
 * roles may be a string or array from { admin, member, recorder }; invalid or
 * empty values default to ['member']. Legacy entries are upgraded on write.
 */
async function setMembership(userId, teamId, roles = ['member']) {
  const teamRoles = normalizeRoles(roles);
  return mutate((users) => {
    const user = users.find(u => u.id === userId);
    if (!user) return null;
    normalizeUserTeams(user);
    const existing = user.teams.find(t => t.teamId === teamId);
    if (existing) {
      existing.roles = teamRoles;
    } else {
      user.teams.push({ teamId, roles: teamRoles });
    }
    user.updatedAt = new Date().toISOString();
    return { teamId, roles: teamRoles };
  });
}
  ...
  return users
    ...
    .map(u => {
      ...
      return {
        ...toPublic(u),
        teamRoles: roles,
        // Back-compat convenience for any single-role consumer.
        teamRole: roles.includes('admin') ? 'admin' : (roles[0] || null)
      };
    });
}
...
async function countTeamRole(teamId, role) {
  ...
  return users.filter(u =>
    ...
    u.teams.some(t => t.teamId === teamId && membershipRoles(t).includes(role))
  ).length;
}
